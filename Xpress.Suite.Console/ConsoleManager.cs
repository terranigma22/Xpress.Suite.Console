...
using System.Text;
using Xpress.Suite.Console.Enums;
using Xpress.Suite.Console.Models;
...
public class ConsoleManager : IDisposable
{
    ...
    public void Log(string message, LogLevel level = LogLevel.Info)
    {
        ...
        var (foreground, background) = level switch
        {
            LogLevel.Warn => (_colorScheme.WarnForeground, _colorScheme.WarnBackground),
            LogLevel.Error or LogLevel.Fatal => (_colorScheme.ErrorForeground, _colorScheme.ErrorBackground),
            LogLevel.Debug => (_colorScheme.DebugForeground, _colorScheme.DebugBackground),
            _ => (_colorScheme.InfoForeground, _colorScheme.InfoBackground)
        };

        var indent = new string(' ', _groups.Count * 2);
        var formattedMessage = $"{indent}{prefix} {DateTime.Now:HH:mm:ss} - {message}";
    ...
    public void Success(string message) => WriteLine($"✓ {message}", ConsoleColor.Green);
    public void Fail(string message) => WriteLine($"✗ {message}", ConsoleColor.Red);
    ...
    public void ProgressBar(int current, int total, int width = 50, string? message = null)
    {
    ...
    {
        BlankLine();
        WriteLine($"╔{'═' * (title.Length + 4)}╗", ConsoleColor.Cyan);
